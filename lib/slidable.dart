...
import 'package:inventree/utils/widgets/slidable/slidable_dismissal.dart';
...
class Slidable extends StatefulWidget {
  ...
  Slidable({
    ...
    Function? onOpen,
    Function? onClose,
    OpenSlidableController? openController,
  }) : this.builder(
          key: key,
          child: child,
  ...
  final OpenSlidableController? openController;
...
class SlidableState extends State<Slidable>
  ...
  void initState() {
    ...
    _initAnimations();
    if (widget.openController != null) {
      widget.openController!.state = this;
    }
  }
  ...
  void open({SlideActionType? actionType, withOpenController = false}) {
    ...
    if (dismissal.resizeDuration == null) {
      ...
      _resizeController =
          AnimationController(duration: dismissal.resizeDuration, vsync: this)
            ...
            ..addStatusListener(_handleAnimation);
...
class OpenSlidableController {
  SlidableState? state;

  OpenSlidableController({this.state});

  Future<void> open({SlideActionType? actionType}) async {
    if (this.state != null) {
      this.state!.open(actionType: actionType, withOpenController: true);
      await Future.delayed(this.state!.widget.movementDuration);
    }
  }

  Future<void> close({SlideActionType? actionType}) async {
    if (this.state != null) {
      this.state!.close();
      await Future.delayed(this.state!.widget.movementDuration);
    }
